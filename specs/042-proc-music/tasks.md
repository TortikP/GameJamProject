...
- [ ] T040 [P1] Pass всех AC из spec — особенно AC-1 (детерминизм), AC-9 (минимум touch'ей), AC-10 (perf). (depends все P1 выше)
...
5. Один из стингов (например `pickup`) — если presets всех мало времени написать.
6. Lead generator (T009) полностью — bass+pad+drums дают базовый «звучит». Lead — топпинг.

**Не режем никогда:** AC-1 (детерминизм), AC-7 (replaceability), AC-9 (zero controller touches). Это инвариант от Andrey'я.
