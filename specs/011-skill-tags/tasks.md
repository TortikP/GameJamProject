...
- [x] **T001** [P1] `scripts/core/skills/skill.gd` — добавить `@export var tags: Array[StringName] = []` после строки `@export var abilities: Array[Ability] = []` (строка 15). Skill API ничего больше не трогать.
- [x] **T002** [P1] `scripts/core/skills/skill_database.gd` — в `_build_skill` после `skill.cooldown = int(...)` (строка 71) вставить блок парсинга `tags` из plan.md §"Парсер snippet". (depends T001)

## Группа B — content (parallel после T002)

- [x] **T003** [P1] [P] `data/skills/skill_debug_punch.json` — добавить `"tags": ["damage"],` после `"cooldown": 0,`. (depends T002)
- [x] **T004** [P1] [P] `data/skills/skill_melee_punch.json` — `"tags": ["damage"],`. (depends T002)
- [x] **T005** [P1] [P] `data/skills/skill_manekin_attack.json` — `"tags": ["damage"],`. (depends T002)
- [x] **T006** [P1] [P] `data/skills/skill_knockback_punch.json` — `"tags": ["damage", "knockback"],`. (depends T002)
- [x] **T007** [P1] **NOT-task:** убедиться что `data/skills/test_*.json` (4 файла) — НЕ изменены. AC-T4. (depends T006)

## Группа C — docs

- [x] **T008** [P2] [P] `specs/007-skill-system/plan.md` — в JSON schema example (строки ~237-261) добавить `"tags": ["damage"],` после `"cooldown": 2,`. Doc-fix только. (depends T001 — концептуально)

## Группа D — verify

- [ ] **T009** [P1] *manual: Egor* — Открыть проект в Godot 4.6.2, F5. Проверить лог: `[INFO][SkillDatabase] loaded N skills` (N = до-PR значение, ничего не отвалилось). Никаких WARN от парсера tags. (depends T002-T007)
- [ ] **T010** [P1] *manual: Egor* — Smoke-тест каста: F1/F2/F3/F4 в godmode → debug skills работают. Manekin → атакует игрока через `skill_manekin_attack` как до PR. AC-T6. (depends T009)
- [x] **T011** [P1] `git diff staging --stat` — проверить что diff = ровно 7 файлов из plan.md, ничего лишнего. (depends T010) — 7 файлов / +13 строк, совпало.
