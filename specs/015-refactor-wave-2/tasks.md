...
- [x] C1. `config/game_speed.cfg` `[ui]` секция — добавить 5 ключей:
  ```
  ...
  ```
- [x] C2. `scripts/presentation/floating_number.gd:11-12` — удалить `const DURATION_MS`, `const CRIT_DURATION_MS`.
- [x] C3. `scripts/presentation/floating_number.gd:_ready` — `var dur_ms` через `GameSpeed.get_value("ui", ..., default)`.
- [x] C4. `scripts/presentation/toast_item.gd:setup` (line 44) — `0.18` → `GameSpeed.get_value("ui", "toast_fade_in_sec", 0.18)`.
- [x] C5. `scripts/presentation/toast_item.gd:_dismiss` (line 52) — `0.20` → `GameSpeed.get_value("ui", "toast_fade_out_sec", 0.20)`.
- [x] C6. `scripts/presentation/toast_layer.gd:12` — удалить `const DEFAULT_DURATION_SEC`.
- [x] C7. `scripts/presentation/toast_layer.gd:_on_request` (line 40) — заменить `DEFAULT_DURATION_SEC` → `GameSpeed.get_value("ui", "toast_default_duration_sec", 2.5)`.
- [x] C8. Manual smoke: floating number, toast — длительности соответствуют cfg, hot-reload (F5 на game_speed.cfg) меняет тайминги без перезапуска. (deferred to Egor)
- [x] C9. Commit: `feat(015): F-009..F-012 — GameSpeed [ui] keys for floating numbers + toasts`.
...
## H — B-001 (self-target AoE preview anchor)

Out-of-band bug-fix докинут в 015 по Egor'ой просьбе (см. spec.md «B-001 add-on»).

- [x] H1. `scripts/core/abilities/ability_target.gd` — добавить виртуальный `preview_anchor_coord(caster_coord, hover_coord) -> Vector2i` (default = hover_coord) с doc-комментарием про rationale.
- [x] H2. `scripts/core/abilities/targets/self_target.gd` — override → `caster_coord`.
- [x] H3. `scripts/presentation/godmode/godmode_controller.gd:326` — внутри AoE preview loop использовать `ab.target.preview_anchor_coord(caster_coord, coord)` (с `null`-guard на `ab.target`).
- [ ] H4. Manual smoke: self-AoE skill в Q-slot, активировать, мышь по полю — зона висит на кастере. Hex-AoE skill — preview снова едет за курсором.
- [x] H5. Commit: `fix(015): B-001 — self-target AoE preview anchors on caster`.

## G — Closeout

- [x] G1. Отметить все `[x]` в этом файле.
- [x] G2. Commit: `docs(015): mark tasks [x]`.
- [x] G3. Push, отдать PR-URL Egor'у в чат.
...
- H независим (3 файла, baseline ability target API + presentation hook).
- Порядок A→B→C→D→E→F→H не строгий, можно параллелить, но коммитим сериями (по кластерам).
