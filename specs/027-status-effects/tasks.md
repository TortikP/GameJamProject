...
- [x] **A1.** Создать `scripts/core/statuses/status_instance.gd` —
...
- [x] **A2.** Создать `scripts/core/statuses/status_runtime.gd` —
...
- [x] **A3.** Создать 9 runtime-классов в `scripts/core/statuses/runtimes/`:
  - [x] `stunned_runtime.gd` — defaults
  - [x] `slowed_runtime.gd` — `modify_speed = floor(s/2)`,
        `override_movement` с rt_flag flip-flop → `(-2,-2)` при rt_flag==1
  - [x] `poisoned_runtime.gd` — `compute_snapshot = args[1] + level * args[2]`,
        `on_turn_start = take_damage(floor(max_hp * snapshot / 100))`
  - [x] `rooted_runtime.gd` — `modify_speed → 0`,
        `override_movement → (-2,-2)`
  - [x] `feared_runtime.gd` — **behavior swap** через `on_apply`/`on_remove`
        (см. plan §"Runtime-классы конкретика"); `on_turn_start` —
        source-validity check; `override_movement` — **НЕ override'ит**.
  - [x] `burning_runtime.gd` — `compute_snapshot = args[1] + level * args[2]`,
        `on_turn_start = take_damage(snapshot)`
  - [x] `glitched_runtime.gd` — pure stub (все defaults)
  - [x] `shielded_runtime.gd` — `compute_snapshot = args[1] + level * args[2]`,
        `damage_reduction = inst.snapshot_value`
  - [x] `enraged_runtime.gd` — **behavior swap** через `on_apply`/`on_remove`
        (то же тело что feared, но behavior_id == "enraged"); `on_turn_start` —
        source-validity check; `override_movement` — **НЕ override'ит**.
- [x] **A4.** Создать `scripts/core/statuses/status_registry.gd` (autoload):
...
- [x] **A5.** Зарегистрировать `StatusRegistry` в `project.godot` autoloads,
  ПОСЛЕ EventBus / GameSpeed / GameLogger.

## B. Status registry data

- [x] **B1.** Создать `data/status_effects/` с 9 файлами:
  - [x] `stunned.json` — family `control`, arity 1, params `[duration]`
  - [x] `slowed.json` — family `debuff`, arity 1, params `[duration]`
  - [x] `poisoned.json` — family `dot`, arity 3,
        params `[duration, damage_pct, lvl_bonus_pct]`
  - [x] `rooted.json` — family `control`, arity 1, params `[duration]`
  - [x] `feared.json` — family `control`, arity 1, params `[duration]`
  - [x] `burning.json` — family `dot`, arity 3,
        params `[duration, damage, lvl_bonus]`
  - [x] `glitched.json` — family `debuff`, arity 1, params `[duration]`
  - [x] `shielded.json` — family `shield`, arity 3,
        params `[duration, n_block, lvl_bonus]`
  - [x] `enraged.json` — family `debuff`, arity 1, params `[duration]`
  - Каждый файл также содержит `loc_name`, `loc_desc` placeholder loc-keys.

## C. Effect-схема — drop duration

- [x] **C1.** `scripts/core/abilities/ability_effect.gd`:
  - удалить `@export var duration`
  - оставить `requires_alive_target`
- [x] **C2.** `scripts/core/abilities/effects/move_effect.gd`:
...
- [x] **C3.** `scripts/core/abilities/effects/damage_effect.gd`,
  `heal_effect.gd`, `create_effect.gd` — удалить TODO-комментарии про DoT/HoT
  (они зависели от duration). Других правок не нужно.
- [x] **C4.** `scripts/core/abilities/effects/status_effect.gd` — переписать:
...
- [x] **D1.** `scripts/core/abilities/ability_database.gd` —
...
- [x] **D2.** Добавить `_parse_status_string(s: String) -> Dictionary`
  (частный, см. plan).
- [x] **D3.** Soft-shim: если в effect-объекте есть ключ `"duration"` —
  `GameLogger.info`, игнорировать. Не warn (после миграции AC-M1 их не
  будет, но info полезен при rebase'ах с других веток).

## E. Actor surface

- [x] **E1.** `scripts/core/actors/actor.gd` — добавить:
...
- [x] **E2.** Добавить метод `tick_statuses_with_ctx(ctx: Dictionary) -> void`
...
- [x] **F2.** Special-case в `_build_target_candidates(actor, selector, ctx)`:
...
- [x] **F3.** Выяснить, читают ли существующие movement-policy
...
- [x] **M1.** Создать `scripts/core/ai/selectors/selector_specific_actor.gd`
  (`class_name SelectorSpecificActor extends TargetSelector`). См.
  plan §"AI scenario building blocks" пункт 1.
- [x] **M2.** Создать `scripts/core/ai/policies/policy_approach_specific_actor.gd`
  (`class_name PolicyApproachSpecificActor extends MovementPolicy`).
- [x] **M3.** Создать `scripts/core/ai/policies/policy_kite_specific_actor.gd`
  (`class_name PolicyKiteSpecificActor extends MovementPolicy`).
- [x] **M4.** `scripts/core/ai/behavior_database.gd` — добавить case'ы:
...
- [x] **M5.** Создать `data/ai_behaviors/feared.json` (kite policy, empty rules,
  empty fallback). См. plan §"AI scenario building blocks" пункт 3.
- [x] **M6.** Создать `data/ai_behaviors/enraged.json` (approach policy,
...
- [x] **G1.** `scripts/presentation/godmode/godmode_controller.gd._on_world_turn_ended`:
...
- [x] **G2.** Добавить `_tick_all_statuses()` метод:
  - `var ctx := _world_ctx()`
  - for each `actor in registry.all()` if alive — `actor.tick_statuses_with_ctx(ctx)`
- [x] **G3.** Add `arena.stun_skip_delay = 0.4` в `config/game_speed.cfg`.

## H. Player input gating при stunned

- [x] **H1.** Найти HUD skill-slot widget (обработчик Q/W/E/R) —
...
- [x] **H2.** Godmode-controller path движения мышью:
...
- [x] **I3.** `unbind()` — disconnect signal, очистка.
- [x] **I4.** `scenes/dev/player.tscn` — добавить child node
...
- [x] **I5.** `scenes/dev/manekin.tscn` — то же.
- [x] **I6.** `scripts/presentation/move_range_overlay.gd:77` — заменить
  `actor.speed` на `actor.effective_speed()`.

## J. Migration JSON

- [x] **J1.** Удалить ключи `"duration"` из всех effect-объектов в:
...
- [x] **J2.** `data/skills/test_combo_hex_circle_damage_status.json`:
  `{"duration":0,"damage":8}, {"duration":2,"status":"burning"}`
  → `{"damage":8}, {"status":"burning(2, 3, 1)"}`.
- [x] **J3.** `data/skills/test_combo_multikey_effect.json`:
  `{"duration":1,"damage":8,"status":"burning"}`
  → `{"damage":8, "status":"burning(1, 2, 1)"}`.
- [x] **J4.** **NEW** `data/skills/test_status_runtime.json` — single
...
- [x] **J5.** Балансные числа `burning(2,3,1)` / `burning(1,2,1)` — в
...
- [x] **L1.** `git add` всё; `git commit` поэтапно (A → B → C → ... → J)
  для читаемой истории. Один коммит = одна логическая единица.
- [x] **L2.** `git push -u origin egor/spec-27-status-effects`
