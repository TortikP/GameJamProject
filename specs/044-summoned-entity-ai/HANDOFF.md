...
4. **Chain-summon** — призванный actor теряет все skills с `&"summon"` в `behaviour_tags` через strip в `CreateEffect._spawn_actor` (правка после первого implement'а по запросу Egor'а). Cap = 1 поколение. Тестовый случай: player саммонит bee'ю → bee'ин `bee_summon_bee` удалён → bee воюет, но не саммонит. Симметрично для enemy-side.
