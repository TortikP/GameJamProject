...
**Эффект «метаморфозы» (короткий, ≤ 2 сек total).** Полностью self-contained в overlay'е: новый шейдер `scripts/presentation/meta/level_transition.gdshader` на fullscreen ColorRect, читает `hint_screen_texture`, делает UV-distort + chromatic split + alpha-fade. Никакого EventBus camera_shake'а (его в проекте нет, не вводим) — «трясёт» сам шейдер через UV-смещение.

1. **Phase A — wobble** (`[meta]/transition_shake_sec`, default 0.4s): шейдер с быстрыми высокочастотными UV-смещениями (псевдо-shake screen-space).
2. **Phase B — distort & fade-out** (`[meta]/transition_distort_sec`, default 0.6s): шейдер пульсирует `distort_amount` 0 → 1.0, `chroma_split` усиливается; параллельно `modulate.a` от 0 до 1 (чёрный fade-out).
