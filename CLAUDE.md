...
| `Array[CustomClass]` (типизованный массив с пользовательским классом) — Godot 4.6 при присваивании `arr[i] = value` делает строгую проверку статического типа. Если значение пришло через Variant-границу (`Dictionary.get()`, duck-typed call на `Node`/`Object` без `as`-каста, динамический `_node.method(value)`), статический тип теряется до `Resource`/`Object` даже при правильном runtime-типе. Падает с `Invalid assignment of index 'N' (on base: 'Array[X]') with value of type 'Resource (X)'`. | Либо используй plain `Array` для слотов/буферов с пользовательскими классами (потеря типобезопасности — мириться), либо везде форсируй `as ClassName` на границах: `return dict.get(id) as Ability`, `var x: SlotBar = node as SlotBar`. Для джемного кода предпочитаем plain `Array` — меньше точек отказа. |
