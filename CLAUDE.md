...
| Ручной `uid="uid://something"` в `.tscn` файлах — Godot 4 UIDs должны быть случайными хешами вида `uid://cbp3et3xvvgm2`. Произвольная строка приводит к тому, что редактор игнорирует файл или берёт старую версию из кеша. | Не указывать `uid=` в tscn вручную — убрать эту часть из заголовка `[gd_scene ...]`, Godot назначит правильный UID сам при первом открытии. |
