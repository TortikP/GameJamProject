...
| **Logger / utility classes**: `class_name Logger` collides with an internal Godot C++ class `Logger` (defined in `core/io/logger.h`). The parser resolves `Logger.method()` to that engine class, not your script, and you get `Static function "info()" not found in base "GDScriptNativeClass"`. Same shape of error appears with autoload registration of utility classes. | For any stateless utility script: don't use `class_name`, don't autoload it, use explicit `const Logger = preload("res://scripts/infrastructure/logger.gd")` at the top of consumers. Methods are `static func name(...)` (multi-line — inline-form static funcs also misbehave in 4.6). This pattern doesn't depend on the global class registry or autoload load order. |
