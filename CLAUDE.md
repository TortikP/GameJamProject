...
| **`super._ready()` НЕ вызывается автоматически** при overrid'е в подклассе. Если `MyPanel extends BasePanel` определяет свой `_ready()` — родительский `_ready()` (со всем setup'ом: resolve_nodes, apply_theme, install handlers, persistence) не отрабатывает вообще. Симптом: фреймворк-фича «работает у эталонной сцены, но не работает у наследника» — handlers не созданы, theme не применен, persistence молчит. То же касается `_process`, `_input`, `_unhandled_input` и других lifecycle-методов. Spec 057 чуть не отгрузил 5 сломанных панелей по этому. | Первой строкой override'а — `super._ready()` (или `super._process(delta)` и т.п.). Голые extends-скрипты без override'а unaffected — Godot вызовет родительский. Применимо к любым framework-классам с нетривиальной lifecycle-логикой (`BasePanel`, любым autoload-наследникам, кастомным `Resource` подклассам с `_init`). |
