...
| `tween.kill()` при активном `await tween.finished` — корутина зависает навсегда, код после `await` не выполняется никогда. | Использовать `tween.custom_step(9999.0)` — промотает твин до конца, `finished` эмитится штатно, `await` резолвится. |
