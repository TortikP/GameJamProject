...
| `Array[StringName]` (и любой typed Array) объявленный в скрипте, но присваиваемый через переменную типа `Object` — Godot запрещает такое присваивание в рантайме ("Invalid assignment of property ... with value of type 'Array' on a base object of type 'RefCounted'"). | Объявлять массивы как `var tags: Array` (без типа) в data-классах, доступ к которым идёт через `Object`-ссылку. Typed arrays работают только когда переменная типизирована как сам класс. |
