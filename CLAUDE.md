...
| Calling instance methods on an autoload (e.g. `Logger.info(tag, msg)`) sometimes errors with `Static function "info()" not found in base "GDScriptNativeClass"`. Godot 4's static analyzer occasionally resolves the autoload name to the script class itself rather than the singleton instance. | For **stateless utilities** (Logger, math helpers), don't use autoload. Use `class_name X` + `extends RefCounted` + `static func` methods. For **stateful** singletons with signals or `_ready`, keep them as autoloads but be aware: if you hit this error, drop the autoload registration and use the class-with-static-methods pattern instead. |
| Inline single-line static functions (`static func name(args): body`) don't reliably register as static — the modifier seems to silently drop, methods become instance-bound, and consumers calling `MyClass.method()` get `Static function "method()" not found in base "GDScriptNativeClass"` even though the class_name and `static` are right there in the source. | Always use multi-line for `static func`. The body goes on the next indented line. Inline form is fine for `func` (instance), broken for `static func`. |
