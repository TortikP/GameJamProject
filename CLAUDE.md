...
| `_unhandled_input` в скрипте на Control-ноде (Panel, VBoxContainer и т.п.) — клики не доходят, потому что сама Control поглощает их раньше через GUI-систему Godot. | Использовать `_input` + `set_process_input` вместо `_unhandled_input` + `set_process_unhandled_input` для Control-нод, которым нужно ловить клики по всему экрану (например, диалоговая панель). |
