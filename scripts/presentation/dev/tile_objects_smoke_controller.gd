...
func _dump_loaded_objects() -> void:
	GameLogger.info("018-smoke", "── TileObjectRegistry sanity ─────────────────")
	...
	for id in _EXPECTED_OBJECTS:
		...
		GameLogger.info("018-smoke", " %-15s level=%2d  blocks_move=%s  breakable=%s  triggers=[on_enter=%s on_turn_end=%s aura=%d on_attacked=%s]  linger=%s  on_destroy_effect=%s  tags=%s" % [
			obj.id, obj.level,
	...
	if not missing.is_empty():
		...
		GameLogger.info("018-smoke", " all 6 expected objects present.")


func _verify_burning_effect() -> void:
	GameLogger.info("018-smoke", "── TileEffectRegistry / burning sanity ───────")
	...
	GameLogger.info("018-smoke", " burning: kind=%s amount=%s duration=%d (expected duration=2)" % [
...
func _on_damaged(coord: Vector2i, hp_remaining: int) -> void:
	GameLogger.info("018-smoke", "EventBus.tile_object_damaged   coord=%s hp=%d" % [str(coord), hp_remaining])


func _on_destroyed(coord: Vector2i, object_id: StringName) -> void:
	GameLogger.info("018-smoke", "EventBus.tile_object_destroyed coord=%s id=%s" % [str(coord), object_id])


func _on_effect_triggered(coord: Vector2i, target_actor_id: StringName, effect_id: StringName) -> void:
	GameLogger.info("018-smoke", "EventBus.tile_object_effect_triggered coord=%s actor=%s effect=%s" % [str(coord), target_actor_id, effect_id])


func _on_actor_exited(coord: Vector2i, actor_id: StringName, object_id: StringName) -> void:
	GameLogger.info("018-smoke", "EventBus.tile_object_actor_exited coord=%s actor=%s object=%s" % [str(coord), actor_id, object_id])
...
func _input(event: InputEvent) -> void:
	...
	match (event as InputEventKey).keycode:
		KEY_F1:
			GameLogger.info("018-smoke", "F1: emitting tile_object_damaged …")
			EventBus.tile_object_damaged.emit(Vector2i(2, 3), 1)
		KEY_F2:
			GameLogger.info("018-smoke", "F2: emitting tile_object_destroyed …")
			EventBus.tile_object_destroyed.emit(Vector2i(2, 3), &"wooden_barrel")
		KEY_F3:
			GameLogger.info("018-smoke", "F3: emitting tile_object_effect_triggered …")
			EventBus.tile_object_effect_triggered.emit(Vector2i(4, 5), &"player", &"damage_zone")
		KEY_F4:
			GameLogger.info("018-smoke", "F4: emitting tile_object_actor_exited …")
			EventBus.tile_object_actor_exited.emit(Vector2i(4, 5), &"player", &"lava_pool")


func _print_help() -> void:
	GameLogger.info("018-smoke", "── Manual signal smoke (F1–F4) ───────────────")
	GameLogger.info("018-smoke", " F1 — tile_object_damaged           F2 — tile_object_destroyed")
	GameLogger.info("018-smoke", " F3 — tile_object_effect_triggered  F4 — tile_object_actor_exited")
	GameLogger.info("018-smoke", " AC-O8 in-scene smoke (lava walk, barrel break, fountain aura)")
	GameLogger.info("018-smoke", " requires T005a (TileSet 'object_id' custom data layer) and a")
	GameLogger.info("018-smoke", " painted TileMap — manual in editor, see specs/018-tile-objects/tasks.md.")
